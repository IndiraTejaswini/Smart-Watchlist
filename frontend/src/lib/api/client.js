 ...
 * it can only do if the error survives the transport layer intact.
 */
...
export async function apiFetch(path, options = {}) {
  ...
  let payload;
  if (USE_MOCK) {
    payload = await handleMock(method, path, body);
  } else {
    try {
      payload = await realFetch(path, method, body, signal);
    } catch (error) {
      if (!isBackendUnavailable(error) || signal?.aborted) throw error;
      payload = await handleMock(method, path, body);
    }
  }
...
function isBackendUnavailable(error) {
  return error instanceof TypeError || error?.name === "NetworkError" || error?.name === "FailedToFetch";
}
